...
export default function TeacherMessagesPage() {
  ...
  const directNotifications = useMemo<ParsedDirectNotification[]>(() => (
    notifications.reduce<ParsedDirectNotification[]>((items, notification) => {
      const parsed = parseDirectMessage(notification.message);
      if (!parsed) return items;
      items.push({ notification, parsed });
      return items;
    }, [])
  ), [notifications]);
