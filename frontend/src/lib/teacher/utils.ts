...
// Default avatar
export const DEFAULT_AVATAR = 'http://localhost:3001/uploads/logo/star_gmail_logo.jpg';
...
export const getEvaluationSortValue = (evaluation: EvaluationRecord): number => {
...
export const formatScore = (averageScore: number | null, ratingScale: number): string => {
  if (averageScore === null) return 'N/A';
  return `${averageScore.toFixed(1)} / ${ratingScale.toFixed(1)}`;
};

export const getStudentStatus = (averageScore: number | null): 'Healthy' | 'Action Needed' | 'No Data' => {
  if (averageScore === null) return 'No Data';
  if (averageScore < 3) return 'Action Needed';
  return 'Healthy';
};
...
export const resolveAvatarUrl = (value: string | null | undefined, fallback: string = DEFAULT_AVATAR): string => {
...
export const parseStudentReplyNotification = (notification: NotificationRecord): StudentReplyRecord | null => {
...
export const toContactType = (role: string): 'Admin' | 'Teacher' | 'Student' => {
  const normalized = String(role || '').trim().toLowerCase();
...
export const setHiddenFeedbackIds = (teacherId: number, ids: number[]): void => {
  localStorage.setItem(`teacher_hidden_feedback_${teacherId}`, JSON.stringify(ids));
};

export const getHiddenMessageIds = (teacherId: number): number[] => {
  try {
    const raw = localStorage.getItem(`teacher_hidden_direct_messages_${teacherId}`);
    const parsed = raw ? JSON.parse(raw) : [];
...
export const setHiddenMessageIds = (teacherId: number, ids: number[]): void => {
  localStorage.setItem(`teacher_hidden_direct_messages_${teacherId}`, JSON.stringify(ids));
};

// ============ Radar Chart & Criteria Helpers ============

export type RadarColor = { key: string; name: string; color: string; fill: string };
...
export const buildRadarData = (
  student: { latestEvaluation: { responses?: EvaluationResponseWithKey[]; rating_scale?: number } | null } | null,
  ...
  const data = activeGlobal.map((criterion, index) => {
    const response = (student?.latestEvaluation?.responses || []).find((r: EvaluationResponseWithKey) => 
...
export const getIcon = (iconName?: string | null, className = 'w-5 h-5') => {
  switch (String(iconName || '').trim()) {
    case 'Home': return createElement(Home, { className });
    case 'Briefcase': return createElement(Briefcase, { className });
    case 'Users': return createElement(Users, { className });
    case 'Users2': return createElement(Users2, { className });
    case 'Heart': return createElement(Heart, { className });
    case 'Smile': return createElement(Smile, { className });
    case 'Brain': return createElement(Brain, { className });
    case 'CreditCard': return createElement(CreditCard, { className });
    case 'Wrench': return createElement(Wrench, { className });
    case 'MessageCircle': return createElement(MessageCircle, { className });
    default: return createElement(Star, { className });
  }
};
