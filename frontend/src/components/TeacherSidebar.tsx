...
export default function TeacherSidebar({ className }: TeacherSidebarProps) {
  ...
  const menuItems = [
    ...
    { icon: Bell, label: 'Notifications', path: '/teacher/notifications', hasNotification: unreadNotificationCount > 0 },
    { icon: MessageSquare, label: 'Messages', path: '/teacher/messages', hasNotification: unreadMessageCount > 0, badgeCount: unreadMessageCount },
  ...
  useEffect(() => {
    ...
    const loadProfileIdentity = async () => {
      try {
        ...
        if (redirectByRole(authUser?.role)) return;
        ...
        if (redirectByRole(data?.role || authUser?.role)) return;
  ...
  useEffect(() => {
    ...
    window.addEventListener('teacher-notifications-updated', loadUnreadNotifications);
    ...
    return () => {
      window.removeEventListener('teacher-notifications-updated', loadUnreadNotifications);
