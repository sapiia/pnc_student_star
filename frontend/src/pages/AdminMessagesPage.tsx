...
export default function AdminMessagesPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (resolvedPhoto) {
        ...
        const savedPhoto = localStorage.getItem(`profile_photo_${resolvedAdminId}`);
        if (savedPhoto) {
          setAdminAvatar(savedPhoto);
        } else {
          setAdminAvatar(`https://picsum.photos/seed/admin-${resolvedAdminId}/100/100`);
        }
      }
    } catch {
      setAdminId(null);
  ...
  useEffect(() => {
    if (!adminId) return;
    try {
      const stored = localStorage.getItem(`admin_hidden_direct_messages_${adminId}`);
  ...
  useEffect(() => {
    if (!adminId) return;
    localStorage.setItem(`admin_hidden_direct_messages_${adminId}`, JSON.stringify(hiddenMessageIds));
  ...
  const contacts = useMemo<Contact[]>(() => {
    ...
    return eligibleUsers.map((user) => {
      ...
      const unreadCount = contactMessages.filter(({ parsed, notification }) => (
        parsed && parsed.fromId === contactId && parsed.toId === adminId && Number(notification.is_read) !== 1
      )).length;
                    ...
                    visibleMessages.map((msg) => (
                      <div key={msg.id} className={cn('flex gap-4 max-w-2xl', msg.isMe ? 'ml-auto flex-row-reverse' : '')}>
                        <div className="size-10 rounded-xl overflow-hidden shrink-0 shadow-sm bg-slate-200">
                          <img
                            src={msg.isMe ? adminAvatar : selectedContact.avatar}
