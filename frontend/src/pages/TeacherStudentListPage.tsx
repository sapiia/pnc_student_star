...
import { getRealtimeSocket, type FeedbackRealtimePayload, type NotificationRealtimePayload } from '../lib/realtime';
...
const RADAR_COLORS = [
  { key: 'score', name: 'Score', color: '#5d5fef', fill: '#5d5fef' },
];

const CRITERIA_COLOR_STYLES = [
  { iconBg: 'bg-indigo-100', iconText: 'text-indigo-600', hover: 'hover:border-indigo-200 hover:shadow-indigo-100', detailText: 'text-indigo-500', stars: 'text-indigo-400' },
  { iconBg: 'bg-emerald-100', iconText: 'text-emerald-600', hover: 'hover:border-emerald-200 hover:shadow-emerald-100', detailText: 'text-emerald-500', stars: 'text-emerald-400' },
  { iconBg: 'bg-amber-100', iconText: 'text-amber-600', hover: 'hover:border-amber-200 hover:shadow-amber-100', detailText: 'text-amber-500', stars: 'text-amber-400' },
  { iconBg: 'bg-rose-100', iconText: 'text-rose-600', hover: 'hover:border-rose-200 hover:shadow-rose-100', detailText: 'text-rose-500', stars: 'text-rose-400' },
  { iconBg: 'bg-cyan-100', iconText: 'text-cyan-600', hover: 'hover:border-cyan-200 hover:shadow-cyan-100', detailText: 'text-cyan-500', stars: 'text-cyan-400' },
  { iconBg: 'bg-violet-100', iconText: 'text-violet-600', hover: 'hover:border-violet-200 hover:shadow-violet-100', detailText: 'text-violet-500', stars: 'text-violet-400' },
];
...
const extractClassName = (user: ApiUser) => {
  ...
  const trimmedSegments = classText.split('-').map((segment) => segment.trim()).filter(Boolean);
...
export default function TeacherStudentListPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (Number.isInteger(resolvedTeacherId) && resolvedTeacherId > 0) {
        ...
        setTeacherName(String(authUser?.name || 'Your teacher').trim() || 'Your teacher');
      }
    } catch {
      setTeacherId(null);
    }
  ...
  const handleSubmitFeedback = async () => {
    ...
    if (trimmedDraft.length > teacherMaxFeedbackCharacters) {
      setFeedbackError(`Feedback must not exceed ${teacherMaxFeedbackCharacters} characters.`);
      return;
    }
    ...
    try {
      if (latestTeacherFeedback) {
        const response = await fetch(`${API_BASE_URL}/feedbacks/${latestTeacherFeedback.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ comment: trimmedDraft })
        });
        const data = await response.json().catch(() => ({}));
  ...
  const confirmDeleteFeedbackForEveryone = async () => {
    ...
    await handleDeleteFeedback(pendingDeleteFeedbackId);
    setPendingDeleteFeedbackId(null);
  };

  const toggleHideFeedback = (feedbackId: number) => {
    setHiddenFeedbackIds((prev) =>
          ...
          <div className="max-w-[1600px] mx-auto">
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                        ...
                        >
                          ...
                          <td className="px-6 py-5">
                            <span className={cn(
                              ...
                              student.averageScore !== null && student.averageScore >= 4 ? "bg-emerald-50 text-emerald-600" : 
                              student.averageScore !== null && student.averageScore >= 3 ? "bg-amber-50 text-amber-600" : "bg-rose-50 text-rose-600"
                            )}>
              ...
              <AnimatePresence mode="wait">
                {isPerformanceOpen && selectedStudent && (
                  <motion.div
                  ...
                  >
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar relative">
                      ...
                      <div className="space-y-4 p-6 border-t border-slate-100 shrink-0 bg-slate-50/30">
                        ...
                        <textarea 
                          ...
                          className="w-full p-4 bg-white border border-slate-200 rounded-2xl text-sm focus:ring-2 focus:ring-primary/10 focus:border-primary outline-none transition-all resize-none"
                        />
                        {feedbackError && (
                          <p className="text-xs font-bold text-rose-500">{feedbackError}</p>
                        )}
                        {feedbackSuccess && (
                          <p className="text-xs font-bold text-emerald-500">{feedbackSuccess}</p>
                        )}
                        <button 
                          onClick={handleSubmitFeedback}
                          disabled={isSubmittingFeedback}
                          className="w-full py-4 bg-primary text-white font-bold rounded-2xl shadow-lg shadow-primary/25 hover:bg-primary/90 transition-all flex items-center justify-center gap-2 disabled:opacity-70"
                        >
