...
export default function TeacherDashboardPage() {
  ...
  const gens = useMemo(
    () => ['All Generations', ...Array.from(new Set(students.map(s => s.generation))).filter(Boolean).sort()],
    [students]
  );
  ...
  const STATS = useMemo(() => {
    ...
    let avgScoreLocal = ratedStudents.length > 0 ? ratedStudents.reduce((acc, curr) => acc + (curr.rating as number), 0) / ratedStudents.length : 0;
    let evalRateLocal = filteredStudents.length > 0 ? (ratedStudents.length / filteredStudents.length) * 100 : 0;
    const needsAttentionCount = ratedStudents.filter(s => (s.rating as number) < 2.5).length;

    return [
      { label: 'Avg Feedback Stars', value: avgScoreLocal.toFixed(1), total: '/5.0', trend: '', icon: Star, color: 'text-amber-500', bg: 'bg-amber-50' },
      { label: 'Evaluation Rate', value: `${Math.round(evalRateLocal)}%`, total: '', trend: '', icon: TrendingUp, color: 'text-primary', bg: 'bg-primary/5' },
