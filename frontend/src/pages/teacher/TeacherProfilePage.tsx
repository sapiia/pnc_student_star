...
export default function TeacherProfilePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedAuth: AuthUser = {
        ...
        role: profileData?.user?.role || authUser.role,
  ...
  useEffect(() => {
    try {
      ...
      if (redirectByRole(user.role || '')) return;
      setAuthUser(user);
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      try {
        ...
        if (redirectByRole(String(data.role || authUser.role || ''))) return;
        ...
        const backendRole = String(data.role || '').trim().toLowerCase();
        if (backendRole && backendRole !== String(authUser.role || '').trim().toLowerCase()) {
          const syncedAuthUser: AuthUser = {
            ...authUser,
            role: backendRole
          };
          setAuthUser(syncedAuthUser);
          localStorage.setItem('auth_user', JSON.stringify(syncedAuthUser));
        }
        setProfileForm((prev) => ({
          ...prev,
          firstName,
